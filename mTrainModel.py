...
from mSaveDirWavXLS import Do_Excel_Dir
...
class Train(Build_model):
    ...
    def load_data(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(images_data,labels, train_size=0.7)
    ...
    def train(self,X_train, X_test, y_train, y_test,model):
        ...
        checkpoint = keras.callbacks.ModelCheckpoint(os.path.join(self.mkdir( os.path.join(self.checkpoints,self.model_name) ),self.model_name+'.h5'),
                                                     ...
                                                     mode='auto',
                                                     period=1)
        checkpoint2 = keras.callbacks.ModelCheckpoint(os.path.join(self.mkdir( os.path.join(self.checkpoints,self.model_name) ),self.model_name+'_ep{epoch:03d}.h5'),
                                                     monitor=config.monitor,
                                                     verbose=1,
        ...
        if self.data_augmentation:
            ...
            history=model.fit_generator(
                ...
                callbacks=[checkpoint, checkpoint2,lr_reduce, tensorboard],
            )
    ...
    def start_train(self):
        ...
        history = self.train(X_train, X_test, y_train, y_test,model)
        return history
...
if __name__=='__main__':
    ...
    shtname ='VGG19'
    Do_Excel_Dir('predict_result.xlsx', shtname).write(1, 1, epochs[None].T)
    Do_Excel_Dir('predict_result.xlsx', shtname).write(1, 2, acc[None].T)
    Do_Excel_Dir('predict_result.xlsx', shtname).write(1, 3, loss[None].T)
    Do_Excel_Dir('predict_result.xlsx', shtname).write(1, 4, val_loss[None].T)
    plt.figure(1)
